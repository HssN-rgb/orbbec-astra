...
class ScreenFieldControlApp:
    ...
    def camera_mapping_loop(self):
            try:
                ...
                try:
                    openni2.initialize(r'C:\Program Files\vscod\OpenNI_2.3.0.86_202210111950_4c8f5aa4_beta6_windows\Win64-Release\sdk\libs')
                    ...
                    if results.pose_landmarks:
                        ...
                        if right_wrist.visibility > 0.5:
                            ...
                            if 0 <= hand_x < depth_array.shape[1] and 0 <= hand_y < depth_array.shape[0]:
                                ...
                                print(f"Raw wrist depth: {int(depth_value)}mm")
                                    ...
                                    if is_inside:
                                        ...
                                        if left_wrist.visibility > 0.5 and 0 <= left_x < depth_array.shape[1] and 0 <= left_y < depth_array.shape[0]:
                                            ...
                                            if left_depth > 0:
                                                ...
                                                # Use the left wrist for depth comparison (landmark 19)
                                                print(f"Left wrist depth: {int(left_depth)}mm, Right wrist depth: {int(depth_value)}mm")
                                                
                                                # CLICK DETECTION: Using both wrist landmarks
                                                should_click = False
                                                
                                                # Define a radius around the right wrist to calculate average depth
                                                radius = 15  # pixels radius around the wrist
                                                avg_depth = 0
                                                pixel_count = 0

                                                # Calculate average depth in the radius around the right wrist
                                                ...
                                                if pixel_count > 0:
                                                    ...
                                                    # Apply click condition with value limiter
                                                    ...
                                                    avg_depth_text = f"R-Wrist:{int(depth_value)}mm Avg:{int(avg_depth)}mm Diff:{int(depth_diff)}mm"
                                                    limiter_text = f"Limiter:{self.depth_value_limiter}mm"
                                                    
                                                    print(f"Wrist depth: {int(depth_value)}mm, Average depth: {int(avg_depth)}mm, " +
                                                    ...
                                                    # Draw circle around wrist with color based on proximity
                                                    ...
                                                    if self.debug_mode:
                                                        ...
                                                        # Show both wrist depths
                                                        wrist_text = f"L-Wrist:{int(left_depth)}mm, R-Wrist:{int(depth_value)}mm"
                                                        cv2.putText(color_image, wrist_text, 
                                                                (hand_x + 10, hand_y + 90), cv2.FONT_HERSHEY_SIMPLEX, 
                                                                0.5, (255, 255, 0), 1, cv2.LINE_AA)
                                                
                                                # Draw line between wrists
                                                cv2.line(color_image, (hand_x, hand_y), (left_x, left_y), (0, 255, 255), 2)
                                                ...
                                                if should_click:
                                                    ...
                                                    # Reset touch state when wrist moves away
                                                    if self.is_touching:
                                            ...
                                            # Only right wrist is visible - show message
