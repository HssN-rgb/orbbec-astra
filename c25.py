...
class ScreenFieldControlApp:
    def __init__(self, root):
        ...
        # Debug toggle button
        self.debug_button = tk.Button(drawing_frame, text="Debug: ON", command=self.toggle_debug_mode)
    ...
    def camera_mapping_loop(self):
        try:
            ...
            while self.mapping_active:
                        ...
                        if 0 <= hand_x < depth_array.shape[1] and 0 <= hand_y < depth_array.shape[0]:
                            ...
                            if depth_value > 0 and self.min_hand_distance <= depth_value <= self.max_hand_distance:
                                ...
                                if is_inside:
                                    ...
                                    # ENHANCED CLICK DETECTION: Find nearest grid point and compare depths
                                    should_click = False
                                    grid_depth_text = ""
                                    
                                    if self.use_grid_depth and len(self.grid_points_3d) > 0:
                                        # Find the nearest grid point to hand position
                                        nearest_grid = self.find_nearest_grid_point((hand_x, hand_y))
                                        self.nearest_grid_point = nearest_grid  # Store for visualization
                                        
                                        if nearest_grid is not None:
                                            # Get depth of the nearest grid point
                                            grid_depth = nearest_grid[2]
                                            
                                            # Calculate absolute difference between hand depth and grid depth
                                            depth_diff = abs(depth_value - grid_depth)
                                            
                                            # Trigger click if hand is close to grid depth
                                            should_click = depth_diff <= self.grid_depth_tolerance
                                            
                                            # Debug information
                                            grid_depth_text = f"Grid:{int(grid_depth)}mm Diff:{int(depth_diff)}mm"
                                            print(f"Grid depth: {int(grid_depth)}mm, Depth diff: {int(depth_diff)}mm")
                                    else:
                                        # Fall back to wall distance if grid depth is not used
                                    ...
                                    # Show grid depth information if available
                                    if grid_depth_text and self.debug_mode:
                                        cv2.putText(color_image, grid_depth_text, 
                ...
                # Display grid depth tolerance if using grid depth
                if self.use_grid_depth:
                    cv2.putText(color_image, f"Grid depth tolerance: {self.grid_depth_tolerance}mm", 
                               (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
                ...
                if len(self.square_points_3d) < 4:
                    ...
                    # Display click mode
                    click_mode = "Grid-based" if self.use_grid_depth else "Wall distance"
                    cv2.putText(color_image, f"Click mode: {click_mode}", 
                               (20, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)
            ...
            if self.show_grid and len(self.grid_points_2d) > 0:
                ...
                if self.wall_plane is not None and hasattr(self, 'wall_angle'):
                    ...
                    if hasattr(self, 'adaptive_thresholds') and self.adaptive_thresholds:
                        ...
                        cv2.putText(image, adaptive_text, (20, 310), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
                    else:
                        cv2.putText(image, "Adaptive mode: OFF", (20, 310), 
